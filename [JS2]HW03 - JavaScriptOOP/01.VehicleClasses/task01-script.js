...
function WaterVehicle(speed, propulsionUnits){
	for(var i = 0; i < propulsionUnits.length; i++){
		if(!(propulsionUnits[i] instanceof Propeller)){
			throw Error("The propulsion units of aircrafts can only be of type Propeller!");
		}		
	}		
	Vehicle.apply(this, arguments);		
}
...
WaterVehicle.prototype.changeSpinDirection = function(){
	for(var fan = 0; fan < this.propulsionUnits.length; fan++){
		if(this.propulsionUnits[i].spinDirectionClockwise){
			this.propulsionUnits[i].spinDirectionClockwise = false;
		}
		else{
			this.propulsionUnits[i].spinDirectionClockwise = true;
		}
	}
}
