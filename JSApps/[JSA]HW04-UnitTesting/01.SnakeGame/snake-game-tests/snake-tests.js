...
module("Food.init");
test("When food is initialized, should have correct position", function () {
    var position = {
        x: 150,
        y: 150
    };
    ...
    equal(position, food.position);
    equal(size, food.size);
});

test("When food changes position, should be correct one", function () {
    var position = {
        x: 150,
        y: 150
    };
    ...
    position.x += 7;
    position.y += 1;
    deepEqual(position, food.position);
    deepEqual(size, food.size);
});
